...
function initListeners() {
	...
	buyEvent.watch(function(error, result) {
		if (!error) {
			...
			encrypted_url = ecies().privateKey(private_keys[accounts.indexOf(consumer)]).publicKey(new bitcore.PublicKey(public_key)).encrypt(decrypted_url);
...
function create_media() {
	...
	var encrypted_url = ecies().privateKey(private_keys[currentAccount]).publicKey(new bitcore.PublicKey(public_key)).encrypt(url);
	encrypted_url = encrypted_url.toString('hex');
	console.log(encrypted_url);
